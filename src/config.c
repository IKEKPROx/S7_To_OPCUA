...
/* Read entire file into a string / 读取整个文件至字符串 */
static char *read_file(const char *path)
{
    FILE *f = fopen(path, "rb");
    if (!f) { fprintf(stderr, "config: 打不开文件 %s\n", path); return NULL; }
...
/* Get a mandatory string field / 获取必填的字符串字段 */
static int get_str(const cJSON *obj, const char *key, char *dst, size_t dstsz)
{
    ...
    if (!cJSON_IsString(it) || it->valuestring == NULL) {
        fprintf(stderr, "config: 缺少字符串字段 \"%s\"\n", key);
        return -1;
    }
    if (strlen(it->valuestring) >= dstsz) {
        fprintf(stderr, "config: 字段 \"%s\" 太长\n", key);
        return -1;
    }
...
/* Get an optional string field; if missing, dst becomes "" / 获取可选字符串字段，缺省置空串 */
static int get_str_opt(const cJSON *obj, const char *key, char *dst, size_t dstsz)
{
    const cJSON *it = cJSON_GetObjectItemCaseSensitive(obj, key);
    if (!it) { dst[0] = '\0'; return 0; }          /* 缺省：当作没填，留空串 */
    if (!cJSON_IsString(it) || it->valuestring == NULL) {
        fprintf(stderr, "config: 字段 \"%s\" 必须是字符串\n", key);
        return -1;
    }
    if (strlen(it->valuestring) >= dstsz) {
        fprintf(stderr, "config: 字段 \"%s\" 太长\n", key);
...
/* Get an integer field / 获取整数字段 */
static int get_int(const cJSON *obj, const char *key, int required,
                   int defval, int *out)
{
    const cJSON *it = cJSON_GetObjectItemCaseSensitive(obj, key);
    if (!it) {
        if (required) { fprintf(stderr, "config: 缺少字段 \"%s\"\n", key); return -1; }
        *out = defval;
        return 0;
    }
    if (!cJSON_IsNumber(it)) {
        fprintf(stderr, "config: 字段 \"%s\" 不是数字\n", key);
        return -1;
    }
    *out = it->valueint;
    return 0;
...
static int parse_tag(const cJSON *jt, TagCfg *tag)
{
    ...
    if (s7_area_from_str(area_str, &tag->area) != 0) {
        fprintf(stderr, "config: tag \"%s\" 的 area 非法: %s\n", tag->name, area_str);
        return -1;
    }
    if (s7_type_from_str(type_str, &tag->type) != 0) {
        fprintf(stderr, "config: tag \"%s\" 的 type 非法: %s\n", tag->name, type_str);
        return -1;
    }
    ...
    if (tag->start < 0) {
        fprintf(stderr, "config: tag \"%s\" 的 start 不能为负: %d\n", tag->name, tag->start);
        return -1;
    }
    if (tag->area == AREA_DB && tag->db < 1) {
        fprintf(stderr, "config: tag \"%s\" 的 db 必须 >=1: %d\n", tag->name, tag->db);
        return -1;
    }
    ...
    if (tag->type == S7_BOOL) {
        if (tag->bit < 0 || tag->bit > 7) {
            fprintf(stderr, "config: BOOL tag \"%s\" 需要 bit(0~7)\n", tag->name);
            return -1;
        }
    }

    /* Optional explicit OPC UA NodeId (e.g. "ns=2;s=[1001001]") / 可选：显式 OPC UA NodeId */
    if (get_str_opt(jt, "node_id", tag->node_id, sizeof(tag->node_id)) != 0) return -1;
    return 0;
}

/* Parse a single PLC / 解析单个 PLC */
static int parse_plc(const cJSON *jp, PlcCfg *plc)
{
    ...
    if (plc->port < 1 || plc->port > 65535) {
        fprintf(stderr, "config: PLC \"%s\" 的 port 非法(1~65535): %d\n", plc->name, plc->port);
        return -1;
    }
    if (plc->poll_interval_ms <= 0) {
        fprintf(stderr, "config: PLC \"%s\" 的 poll_interval_ms 必须 >0: %d\n",
                plc->name, plc->poll_interval_ms);
        return -1;
    }
    if (plc->rack < 0 || plc->slot < 0) {
        fprintf(stderr, "config: PLC \"%s\" 的 rack/slot 不能为负\n", plc->name);
        return -1;
    }

    const cJSON *jtags = cJSON_GetObjectItemCaseSensitive(jp, "tags");
    if (!cJSON_IsArray(jtags)) {
        fprintf(stderr, "config: PLC \"%s\" 缺少 tags 数组\n", plc->name);
        return -1;
    }
    int n = cJSON_GetArraySize(jtags);
    if (n <= 0) {
        fprintf(stderr, "config: PLC \"%s\" 的 tags 为空\n", plc->name);
        return -1;
    }
...
int config_load(const char *path, GatewayCfg *out)
{
    ...
    free(text);
    if (!root) {
        fprintf(stderr, "config: JSON 解析失败 (在 %s 附近)\n",
    ...
    if (!cJSON_IsArray(jplcs)) {
        fprintf(stderr, "config: 缺少 plcs 数组\n");
        goto done;
    }
    int n = cJSON_GetArraySize(jplcs);
    if (n <= 0) {
        fprintf(stderr, "config: plcs 为空\n");
        goto done;
    }
