...
/* Decode 16-bit unsigned integer (big-endian) / 解码 16 位无符号整数（大端序） */
static uint16_t be_u16(const uint8_t *b)
{
    return (uint16_t)(((uint16_t)b[0] << 8) | (uint16_t)b[1]);
}

/* Decode 32-bit unsigned integer (big-endian) / 解码 32 位无符号整数（大端序） */
...
/* Decode 64-bit unsigned integer (big-endian) / 解码 64 位无符号整数（大端序） */
...
int s7_decode(S7DataType t, const uint8_t *buf, int bit, S7Value *out)
{
    ...
    switch (t) {
        ...
        case S7_REAL: {
            /* Decode IEEE 754 Float using memcpy to avoid strict-aliasing violations / 使用 memcpy 解码 IEEE 754 浮点数，避免 strict-aliasing 违规 */
