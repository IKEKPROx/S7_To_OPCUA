/*
 * test_config —— 加载 config/gateway.json，验证解析出来的内容对不对。
 * 还顺手测一个【故意写错的】JSON，确认错误能被拦下来。
 * 从项目根目录运行（make 会保证这一点），所以路径写相对的 config/...
 */
#include "config.h"
#include <stdio.h>
#include <string.h>
...
int main(void)
{
    printf("== 加载正常配置 config/gateway.json ==\n");
    GatewayCfg cfg;
    ...
    /* 第 2 个 PLC：M 区 INT，poll 默认覆盖为 500 */
