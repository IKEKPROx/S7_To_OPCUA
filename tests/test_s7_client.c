/*
 * test_s7_client —— 不需要真实 PLC！
 *
 * 思路：snap7 自带 server 功能，我们在本机内存里起一个【假 PLC】，
 * 注册一个 DB10、填进已知的大端字节，然后用我们写的 s7_client 去连它、
 * 读 tag，验证读出来的值对不对。完全本地、可重复。
 *
 * 用端口 1102 而不是标准 102，免得需要 root 权限。
 */
#include "s7_client.h"
#include "config.h"
#include "snap7.h"
...
int main(void)
{
    ...
    /* DB10 的内容，按 S7 大端手工摆好：
     *   start 0 : REAL 1.5     -> 3F C0 00 00
     *   start 4 : BOOL bit0=1  -> 01
     *   start 6 : DINT 256     -> 00 00 01 00            */
    ...
    /* 自动找一个能用的端口：上次的 server 没退干净、或别的程序占了 1102
     * 都很常见。在 1102~1110 里挨个试，找到第一个能起来的。
     * 失败时打印 snap7 的真实错误，方便排查（而不是静默跳过）。 */
    int chosen_port = 0;
