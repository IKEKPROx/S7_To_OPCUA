...
GW_SRC = src/main.c src/config.c src/s7_types.c src/s7_client.c \
         src/tag_cache.c src/opcua_server.c src/poll_engine.c
...
# opcua_server 层：依赖 open62541（+ tag_cache + s7_types）
test_opcua_server: tests/test_opcua_server.c src/opcua_server.c src/tag_cache.c src/s7_types.c
	$(CC) $(CFLAGS) $(OPCUA_CFLAGS) $^ $(OPCUA_LIBS) -lpthread -lm -o build_test_opcua_server
